...
{path_to_repository}\extra_files\setup_files\windows
```
You should now move the *Time Stamper* program files that you previously generated using *Auto PY to EXE* into the appropriate folder. **Assuming** that you generated your *Time Stamper* executable (.exe) in one "One Directory" mode (as explained in the above paragraph), then you should perform the following steps:
...
* Paste the contents of this folder into the following directory, replacing {version_number} with the version of the *Time Stamper* program the source code you downloaded is associated with
    ```
