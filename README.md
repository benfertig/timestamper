...
**NOTE:** You may run into a glitch where the icon for the TimeStamper.exe file you generated will not change even if you set a custom icon **UNLESS** you change the name of the .exe file. Why this glitch occurs is unclear, but it is not a big deal. Just change the name of the TimeStamper.exe to literally anything else. You can even make a change as small as calling the file "Time Stamper.exe" or "timestamper.exe" instead of "TimeStamper.exe".<br />
